...
def index():
    ...
    if form.validate_on_submit():
        # post = Post(body=form.post.data, author=None)
        # db.session.add(post)
        # db.session.commit()
        flash('Your image was uploaded!')
        # filename = url_for('static', filename='img3.jpg')
        # filename = filename.replace('/','_')
        filename = 'img3.jpg'
        lang = 'es'
        s = True
        return redirect(url_for('detect', imagename=filename, lang=lang, speech=s))
...
@app.route('/detect/<imagename>/<lang>/<speech>')
def detect(imagename, lang='en', speech=False):
    flash(f'Your image is {imagename}')
    landmarks = detect_landmarks('app/static/'+imagename)
    ...
    image_path = 'img3_square.jpg'
    return render_template('detect.html', title='Image Detection',
                           landmark=landmark, image_path=image_path)
